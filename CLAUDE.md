...
- **Squad detail layout:** Uses `.squad-detail-layout` (3-col: `1fr 1fr 30%`) not `.detail-layout`. Column 2 stacks Projects then Repos (1 rem gap; repos hidden when none). Repos shown if directly `owned-by` squad OR `belongs-to` any squad-owned project. No cross-highlight dim interaction — removed in favour of EntityPopup drill-down. See [SPEC-007](spec/007-squad-detail.md).
